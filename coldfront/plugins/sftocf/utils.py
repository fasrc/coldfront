...
SERVER_JSON = 'coldfront/plugins/sftocf/servers.json'
...
class StarFishRedash:
    ...
    def submit_query(self, queryname):
        ...
        if isinstance(query[0], int):
            ...
            return return_get_json(query_url, headers={})
        result_data_rows = []
        for q in query:
            query_url = f'{self.base_url}queries/{q[0]}/results?api_key={q[1]}'
            r = return_get_json(query_url, headers={})
            result_data_rows.extend(r['query_result']['data']['rows'])
        result = r
        result['query_result']['data']['rows'] = result_data_rows
        return result
...
class RedashDataPipeline(UsageDataPipelineBase):
    ...
    def collect_sf_user_data(self):
        ...
        # {'size_sum': 912956, 'lab_path': 'rc_labs/capasso_lab', 'vol_name': 'holy-isilon', 'user_name': 'vsanjeev'},
        return user_usage
    ...
    def collect_sf_usage_data(self):
        ...
        #  {'volume_id': 15, 'vol_name': 'bos-isilon', 'gid': 0, 'group_name': 'root', 'uid': 0, 'user_name': 'root', 'path': 'rc_labs/oeb_administration', 'total_size': 94481425278}
